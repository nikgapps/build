            ...
            if git_check:
                ...
                if release_repo is not None:
                    release_datetime = release_repo.get_latest_commit_date(branch="master",
                                                                           filter_key=android_version_str)
                    print(f"Last Release ({android_version_str}): " + str(release_datetime))
                apk_source_datetime = GitOperations.get_last_commit_date(android_version=android_version_str,
                                                                         branch="main" if Config.RELEASE_TYPE.__eq__(
                                                                             "stable") else "canary")
                if apk_source_datetime is not None:
                    print("Last Apk Repo (" + android_version_str + ") Commit: " + str(apk_source_datetime))
                    if float(android_version) >= 12.1:
                        GitOperations.clone_overlay_repo(android_version, fresh_clone=False, branch="master")
                else:
                    print(Statics.pwd + Statics.dir_sep + android_version_str + " doesn't exist!")
